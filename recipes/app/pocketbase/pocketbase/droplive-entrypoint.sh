...
# droplive: generate=hex96 ownership=app purpose=owner-bootstrap lifecycle=stable rotation=app name=PB_ADMIN_PASSWORD capability=owner-login username=droplive-owner@local.droplive.test
...
if [ ! -e /pb_data/data.db ]; then
  ...
  /usr/local/bin/pocketbase superuser create \
    droplive-owner@local.droplive.test \
    "$PB_ADMIN_PASSWORD" \
    --dir=/pb_data
fi
